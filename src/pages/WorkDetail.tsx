...
const FAB_MENU: { type: EventType; label: string; icon: typeof Plus; description: string }[] = [
  {
    ...
    icon: RefreshCw,
    description: "Record changes to the artwork's physical condition",
  },
  {
    type: 'location_change',
    label: 'Location Change',
    icon: MapPin,
    description: 'Log where the work has been moved to',
  },
  {
    type: 'status_change',
    label: 'Status Change',
    icon: RefreshCw,
    description: 'Update intake, display, storage, or sold status',
  },
  {
    type: 'note',
    label: 'Note',
    icon: MessageSquare,
    description: 'Add a freeform observation or memo',
  },
  {
    type: 'sale',
    label: 'Sale',
    icon: DollarSign,
    description: 'Record sale price and commission details',
  },
  {
    type: 'payout',
    label: 'Payout',
    icon: CreditCard,
    description: 'Log a payment made to the consignor',
  },
  {
    type: 'document_attach',
    label: 'Attach Document',
    icon: Paperclip,
    description: 'Attach a contract, receipt, or certificate',
  },
]
...
export default function WorkDetail() {
  ...
  useEffect(() => {
    ...
    const unsub1 = subscribeToWork(gallery.id, workId, (w) => {
      setWork(w)
      setLoading(false)
    })
  ...
  useEffect(() => {
    if (!gallery || !work?.consignorId) {
      setConsignorName(null)
      return
    }
  ...
  const currentCondition = useMemo(() => {
    const conditionEvents = events.filter(
      (e) => e.type === 'condition_update' || e.type === 'intake'
    )
  ...
  const consignorShare =
    work?.salePrice != null && work?.commissionRate != null
  ...
  async function handleExport(mode: 'full' | 'selective') {
    ...
    try {
      ...
      const filename = `${work.artist}_${work.title}_provenance.pdf`.replace(
        /[^a-zA-Z0-9_.-]/g,
        '_'
      )
    ...
    return (
      <div>
        <div className="sticky top-0 z-40 flex h-14 items-center border-b border-border/50 bg-background/95 backdrop-blur-xl px-4">
          <button
            onClick={() => navigate(-1)}
            className="mr-2 flex h-11 w-11 items-center justify-center text-muted-foreground hover:text-gold"
          >
  ...
  return (
    <div>
      ...
      <div className="sticky top-0 z-40 flex h-14 items-center border-b border-border/50 bg-background/95 backdrop-blur-xl px-4">
        <button
          onClick={() => navigate(-1)}
          className="mr-2 flex h-11 w-11 items-center justify-center text-muted-foreground hover:text-gold transition-colors"
        >
          <ArrowLeft className="h-5 w-5" strokeWidth={1.5} />
        </button>
        ...
        <button
          ...
          aria-label="Export provenance PDF"
          data-tour="export-provenance"
          className="flex h-11 items-center justify-center gap-1.5 px-2 text-muted-foreground hover:text-gold transition-colors"
        >
        ...
        <div className="lg:w-1/2 lg:min-h-screen lg:overflow-y-auto pb-24">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="mt-0.5 text-foreground">
                  {currentCondition
                    ? CONDITION_LABELS[currentCondition as ConditionRating] || currentCondition
                    : 'Not recorded'}
                </p>
              </div>
            </div>
          ...
          <div className="px-4 lg:px-8 pb-2">
            <GuidanceTip id="work-detail-timeline">
              Every change to this work is recorded in the timeline below. Use the + button to log
              condition updates, location moves, sales, and more.
            </GuidanceTip>
          </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="mt-0.5 text-xs text-muted-foreground">
                        Record where this work is currently stored
                      </p>
                    </div>
                    <span className="shrink-0 text-[10px] font-medium uppercase tracking-wider text-gold/70">
                      Add
                    </span>
                  </button>
                </div>
                ...
                <div className="px-4 lg:px-8 pb-4">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="mt-0.5 text-xs text-muted-foreground">
                        Log the payment made to the consignor for this sale
                      </p>
                    </div>
                    <span className="shrink-0 text-[10px] font-medium uppercase tracking-wider text-gold/70">
                      Add
                ...
                <div className="px-4 lg:px-8 pb-4">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="mt-0.5 text-xs text-muted-foreground">
                        Log a condition update or location change to begin the record
                      </p>
                    </div>
                    <span className="shrink-0 text-[10px] font-medium uppercase tracking-wider text-gold/70">
                      Add
                ...
                {work.commissionRate != null && (
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">
                      Gallery commission ({Math.round(work.commissionRate * 100)}%)
                    </span>
                    <span>
                      {formatCurrency(
                        Math.round(work.salePrice * work.commissionRate),
                        work.currency
                      )}
                    </span>
                  </div>
          ...
          >
            ...
            {events.filter((e) => e.type !== 'intake').length === 0 && (
              <p className="text-sm text-muted-foreground mb-4">
                No events recorded yet. Use the + button below to start building this work's
                provenance trail.
              </p>
            )}
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-muted-foreground">
                            ...
                            <span className="text-gold/80">{EVENT_TYPE_LABELS[event.type]}</span>
                          </span>
                          {editable && (
                            <button
                        ...
                        {event.type === 'sale' && details?.salePrice && (
                          <p className="text-xs text-muted-foreground">
                            ...
                            {details.commissionRate != null &&
                              ` (${Math.round(details.commissionRate * 100)}% commission)`}
                            {details.buyerName && ` — Buyer: ${details.buyerName}`}
                          </p>
                        )}
                        {event.type === 'payout' && details?.amount && (
                          <p className="text-xs text-muted-foreground">
                            {formatCurrency(details.amount, details.currency || 'USD')} via{' '}
                            {details.method}
      ...
      >
        <SheetContent side="bottom" className="max-h-[85vh] overflow-y-auto">
          <SheetHeader>
            <SheetTitle>Export Provenance Pack</SheetTitle>
          </SheetHeader>
          <p className="mt-2 text-sm text-muted-foreground px-0">
            Generate a PDF provenance report with cover photo, condition history, timeline events,
            and financial summary. Choose full export or select specific events.
          </p>
          {!exportMode ? (
            <div className="space-y-2 pt-4">
              ...
              >
                ...
                <span className="text-xs text-muted-foreground mt-1">
                  All events, photos, condition history, and financials
                </span>
              </button>
              <button
                onClick={() => {
            ...
            <div className="space-y-3 pt-4">
              ...
              <div className="space-y-1 max-h-[50vh] overflow-y-auto">
                {events.map((ev) => (
                  <label
                    key={ev.id}
                    className="flex items-center gap-3 p-2 transition-colors hover:bg-secondary cursor-pointer"
                  >
                    <input
                      type="checkbox"
...
function EventFormSheet({
  ...
  const payoutTotal = events
    .filter((e) => e.type === 'payout')
    .reduce((s, e) => s + ((e.details as any)?.amount ?? 0), 0)
  const consShare =
    work.salePrice != null && work.commissionRate != null
      ? Math.round(work.salePrice * (1 - work.commissionRate))
      : 0
      ...
      switch (type) {
        case 'condition_update': {
          ...
          await createEvent(galleryId, work.id, 'condition_update', desc, {
            conditionSummary: condRating,
            conditionNotes: condNotes.trim(),
          })
          break
        }
        case 'location_change': {
          if (!locTo.trim()) {
            toast.error('Enter the new location')
            setSaving(false)
            return
          }
        ...
        case 'status_change': {
          ...
          await createStatusChangeEvent(
            galleryId,
            work.id,
            work.status,
            newStatus as WorkStatus,
            desc
          )
          break
        }
        ...
        case 'sale': {
          ...
          if (isNaN(commNum) || commNum < 0 || commNum > 100) {
            toast.error('Enter commission rate (0-100)')
            setSaving(false)
            return
          }
            ...
            <>
              {events.some((e) => e.type === 'sale') && (
                <div className="border border-warning/20 bg-warning/5 p-3 text-sm text-warning">
                  This work already has a sale recorded. A new sale will update the financial
                  details.
                </div>
              )}
              <div className="space-y-2">
            ...
            <>
              {work.salePrice != null ? (
                <div className="border border-border/50 bg-card p-3 space-y-1 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">
                      Commission (
                      {work.commissionRate != null ? Math.round(work.commissionRate * 100) : 0}%)
                    </span>
                    <span>
                      -
                      {formatCurrency(
                        Math.round(work.salePrice * (work.commissionRate ?? 0)),
                        work.currency
                      )}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Consignor share</span>
                    <span>{formatCurrency(consShare, work.currency)}</span>
                  </div>
                  {payoutTotal > 0 && (
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Previously paid</span>
                      <span>-{formatCurrency(payoutTotal, work.currency)}</span>
                    </div>
                  )}
                  <div className="h-px bg-border/50 my-1" />
                  <div className="flex justify-between font-medium">
                    <span>Remaining</span>
                    <span>{formatCurrency(Math.round(remaining), work.currency)}</span>
                  </div>
                </div>
              ) : (
                <div className="border border-warning/20 bg-warning/5 p-3 text-sm text-warning">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0.01"
                  value={payoutAmtStr}
                  onChange={(e) => setPayoutAmtStr(e.target.value)}
                  className="h-11 bg-card border-border/60"
                  placeholder="0.00"
                />
              </div>
...
function EditWorkSheet({
  ...
  useEffect(() => {
    if (open) {
      setArtist(work.artist)
      setTitle(work.title)
      setMedium(work.medium)
  ...
  return (
    ...
    >
      <SheetContent side="bottom" className="max-h-[85vh] overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Edit Work Details</SheetTitle>
        </SheetHeader>
        <div className="space-y-4 pt-4">
          <div className="space-y-2">
            <Label className="text-xs uppercase tracking-widest text-muted-foreground">
